...
function abrirAnexo(anexo) {
  // Abre em janela isolada (noopener: a nova aba não recebe referência a esta
  // janela, evitando que um anexo malicioso redirecione a aba original) e usa
  // um iframe em sandbox para o PDF, sem permitir scripts nem navegação —
  // mesmo que o "tipo" do arquivo tenha sido informado de forma incorreta ou
  // adulterada, nada executa dentro dele.
  const win = window.open("", "_blank", "noopener");
  if (!win) { toast("Bloqueador de pop-ups ativo. Permita pop-ups para visualizar."); return; }
  if (anexo.tipo === "application/pdf") {
    win.document.write(`<iframe src="${anexo.dataURL}" sandbox="allow-same-origin" referrerpolicy="no-referrer" style="border:none;width:100%;height:100%;"></iframe>`);
  } else {
    win.document.write(`<img src="${anexo.dataURL}" referrerpolicy="no-referrer" style="max-width:100%;">`);
  }
}
